...
def criar_meta(payload: MetaCreate, db: Session = Depends(get_db)) -> Meta:
...
def atualizar_meta(meta_id: int, payload: MetaUpdate, db: Session = Depends(get_db)) -> Meta:
    meta = db.get(Meta, meta_id)
    if meta is None:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Meta não encontrada")
...
def remover_meta(meta_id: int, db: Session = Depends(get_db)) -> None:
    meta = db.get(Meta, meta_id)
    if meta is None:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Meta não encontrada")
    db.delete(meta)
